...
const Shielder = () => {
  ...
  const { value: currentTab, set: setCurrentTab } =
    useLocalStorageValue<keyof typeof TABS>('tab', { defaultValue: 'tokens' });
  ...
  useEffect(() => {
    ...
    if (currentTab && !validKeys.includes(currentTab)) {
      setCurrentTab(validKeys[0]);
    }
  }, [currentTab, setCurrentTab, tabsConfig]);
